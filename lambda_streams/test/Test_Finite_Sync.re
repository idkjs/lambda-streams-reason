...
let rec finite_sync_fmt = (~none as pp_none=Fmt.nop, pp_v, ppf, value) =>
  switch (Sync.next(value)) {
  | Signal.Data(value') =>
    pp_v(ppf, value');
...
let finite_sync = e => {
  let rec eq = (x, y) =>
    switch (Sync.next(x), Sync.next(y)) {
    | (Signal.Data(a), Signal.Data(b)) =>
      Alcotest.equal(e, a, b) && eq(x, y)
...
let test_arrays_not_transducers = _ => {
  let filter' = (f, x) =>
    x |> Array.to_list |> List.filter(f) |> Array.of_list;
  let invocations = ref([]);
  let result =
    [|1, 2, 3|]
    |> Array.map(e => {
...
let test_from_list = _ => {
  Alcotest.(check(signal(int)))(
    "",
    Finite.Sync.from_list([]) |> Sync.next,
    EndOfSignal,
  );

  let stream = Finite.Sync.from_list([1, 2, 3]);
...
let test_from_array = _ => {
  Alcotest.(check(signal(int)))(
    "",
    Finite.Sync.from_array([||]) |> Sync.next,
    EndOfSignal,
  );

  let stream = Finite.Sync.from_array([|1, 2, 3|]);
  Alcotest.(check(signal(int)))("", Sync.next(stream), Data(1));
  Alcotest.(check(signal(int)))("", Sync.next(stream), Data(2));
  Alcotest.(check(signal(int)))("", Sync.next(stream), Data(3));
  Alcotest.(check(signal(int)))("", Sync.next(stream), EndOfSignal);
...
let test_filter = _ => {
  ...
  let stream =
    Sync.enumerate() |> Finite.Sync.take'(10) |> Finite.Sync.filter(is_even);
...
let test_until = _ => {
  let stream = Finite.Sync.pure(123) |> Finite.Sync.until((<)(3));
  Alcotest.(check(signal(int)))("", Sync.next(stream), Data(123));

  let stream2 = Finite.Sync.pure(1) |> Finite.Sync.until((<)(3));
  Alcotest.(check(signal(int)))("", Sync.next(stream2), EndOfSignal);

  let stream3 =
    Finite.Sync.from_array([|1, 2, 3|]) |> Finite.Sync.until((>)(3));
...
let test_fold_left = _ =>
  Alcotest.(check(int))(
    "",
    Finite.Sync.from_array([|1, 2, 3|]) |> Finite.Sync.fold_left((+), 0),
    6,
  );
