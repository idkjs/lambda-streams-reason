...
let test_first_to_lwt = (_, ()) =>
  ...
  >>= (value => Alcotest.(check(string))("", value, "foo") |> Lwt.return);

let test_last_to_lwt = (_, ()) =>
  Lambda_streams.Finite.Async.from_list(["foo", "bar", "baz"])
  |> Lambda_streams_lwt.Async.last_to_lwt
  >>= (value => Alcotest.(check(string))("", value, "baz") |> Lwt.return);
...
let test_to_lwt_list = (_, ()) =>
  ...
  >>= (
    value =>
      Alcotest.(check(list(string)))("", value, ["foo", "bar", "baz"])
      |> Lwt.return
  );
...
let test_from_lwt = (_, ()) =>
  ...
  |> Lambda_streams.Finite.Async.map(value =>
       Alcotest.(check(string))("", value, "foo")
     )
  |> Lambda_streams_lwt.Async.last_to_lwt;

let test_to_lwt_stream = (_, ()) => {
  let expected_list = ["foo", "bar", "baz"];
  Lambda_streams.Finite.Async.from_list(expected_list)
  |> Lambda_streams_lwt.Async.to_lwt_stream
  |> Lwt_stream.to_list
  >>= (
    list =>
      Alcotest.(check(list(string)))("", list, expected_list) |> Lwt.return
  );
};
  ...
  Alcotest_lwt.run(
    ...
    [
      (
        ...
        |> List.map(test => Alcotest_lwt.test_case("", `Quick, test)),
      ),
    ],
  );
};
