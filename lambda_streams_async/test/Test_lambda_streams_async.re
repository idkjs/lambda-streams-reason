...
let test_first_to_async = _ =>
  ...
  >>= (value => Alcotest.(check(string))("", value, "foo") |> Async.return);

let test_last_to_async = _ =>
  Lambda_streams.Finite.Async.from_list(["foo", "bar", "baz"])
  |> Lambda_streams_async.Async.last_to_async
  >>= (value => Alcotest.(check(string))("", value, "baz") |> Async.return);
...
let test_from_async = _ =>
  ...
  |> Lambda_streams_async.Async.first_to_async
  >>= (value => Alcotest.(check(int))("", value, 246) |> Async.return);
    ...
    Alcotest_async.run(
      ...
      [
        (
          ...
          |> List.map(test => Alcotest_async.test_case("", `Quick, test)),
        ),
      ],
    );
